...
if [[ -z ${category} ]] || [[ -z ${version} ]] || [[ -z ${dataSource} ]] || [[ -z ${output} ]]; then
	usage
fi

# env
PROJECT_BASE="/Users/sindongboy/Dropbox/Documents/workspace/digital-contents-unifier"
RESOURCE_BASE="/Users/sindongboy/Dropbox/Documents/resource/digital-contents-unifier"
CONFIG="${PROJECT_BASE}/config"
DICT="${RESOURCE_BASE}/dict"
RESOURCE_CRAWL_PATH=`find ${RESOURCE_BASE}/rsync/crawl -type d -name "*" | awk '{printf("%s:", $0)}' | sed 's/:$//g'`
RESOURCE_SERVICE_PATH=`find ${RESOURCE_BASE}/resource -type d -name "*" | awk '{printf("%s:", $0)}' | sed 's/:$//g'`
...
# Target
TARGET="${PROJECT_BASE}/target/digital-contents-unifier-1.1.0-SNAPSHOT.jar"
...
java -Xmx4G -cp ${CP} com.skplanet.nlp.unifier.dc.driver.CreateSourceIDMap -c ${category} -v ${version} -u ${dataSource} -o ${output}
